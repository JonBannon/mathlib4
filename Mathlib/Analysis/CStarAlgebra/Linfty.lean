...
/-!
# Linfty is a CStarAlgebra

In this file we provide a `CStarAlgebra (Lp R ∞ μ)` instance.

*I think I should run the code below through the GPT to try to find improvements/edits*
*then as I do these I can update the module docstring with details of the auxiliary*
*declarations. Then I can PR the whole thing.*
...
noncomputable instance : Distrib (Lp R ∞ μ) where
  left_distrib := by
    ...
    rw [h3, Pi.add_apply, h4, h5, h1, Pi.mul_apply, h2, Pi.add_apply,
      Pi.mul_apply, Pi.mul_apply, mul_add]
  right_distrib := by
    intro f g h
    ext
...
instance : StarMul (α →ₘ[μ] R) where
  star_mul f g := by
    ext
    filter_upwards [AEEqFun.coeFn_star (f * g), AEEqFun.coeFn_mul f g,
      AEEqFun.coeFn_mul (star g) (star f), AEEqFun.coeFn_star f, AEEqFun.coeFn_star g]
        with x hx hy hz h1 h2
    simp [hx, Pi.star_apply, hy, Pi.mul_apply, hz, h1, h2, star_mul]

instance : StarAddMonoid (α →ₘ[μ] R) where
  star_add f g:= by
    ext
    filter_upwards [AEEqFun.coeFn_star (f + g), AEEqFun.coeFn_add (star f) (star g),
       AEEqFun.coeFn_add f g, AEEqFun.coeFn_star f, AEEqFun.coeFn_star g] with x hx hy hz hq hw
...
noncomputable instance : StarMul (Lp R ∞ μ) where
  star_mul f g := by
    ...
    filter_upwards [Lp.coeFn_star (f * g), Linfty.coeFn_mul f g,
      Linfty.coeFn_mul (star g) (star f), Lp.coeFn_star f, Lp.coeFn_star g]
...
noncomputable instance : StarAddMonoid (Lp R ∞ μ) where
  star_add f g := by
    ext
    filter_upwards [Lp.coeFn_add f g, Lp.coeFn_star (f + g), Lp.coeFn_add (star f) (star g),
       Lp.coeFn_star f, Lp.coeFn_star g] with x hx hy hz hw hq
    rw [hy, Pi.star_apply, hx, Pi.add_apply, star_add, hz, Pi.add_apply, hw, hq, Pi.star_apply,
       Pi.star_apply]
...
lemma enorm_le_of_ae_enorm_le (f : Lp R ∞ μ) (c : ℝ≥0∞) (hf : ∀ᵐ (x : α) ∂μ, ‖f x‖ₑ ≤ c) :
     ‖f‖ₑ ≤ c := by
  have := essSup_le_of_ae_le _ hf
  simpa [Lp.enorm_def, eLpNorm_exponent_top, ge_iff_le]

lemma nnnorm_le_of_ae_nnnorm_le (f : Lp R ∞ μ) (c : ℝ≥0) (hf : ∀ᵐ (x : α) ∂μ, ‖f x‖₊ ≤ c) :
     ‖f‖₊ ≤ c := by
  have hf' : ∀ᵐ x ∂μ, ‖f x‖ₑ ≤ c := by filter_upwards [hf]; simp
  simpa only [enorm_le_coe] using enorm_le_of_ae_enorm_le f c hf'

lemma norm_le_of_ae_norm_le (f : Lp R ∞ μ) (c : ℝ) (hc : 0 ≤ c) (hf : ∀ᵐ (x : α) ∂μ, ‖f x‖ ≤ c) :
     ‖f‖ ≤ c :=
...
instance [StarRing R] [CStarRing R] : CStarRing (Lp R ∞ μ) where
  norm_mul_self_le f := by
    ...
    filter_upwards [ae_norm_le_norm (star f * f), Lp.coeFn_star f, Linfty.coeFn_mul (star f) f]
       with x hx hx_star hx_mul
...
noncomputable instance : StarModule 𝕜 (α →ₘ[μ] R) where
  star_smul := by
     ...
     filter_upwards [AEEqFun.coeFn_star (c • f), AEEqFun.coeFn_smul c f,
       AEEqFun.coeFn_smul (star c) (star f) |>.symm, AEEqFun.coeFn_star f]
         with x hstar1 hsmul1 hsmul2 hstar2
